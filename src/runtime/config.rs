...
/// Configuration for an optional quality pipeline.
/// Parsed from the `"pipeline"` section of `.claudette.json`.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PipelineConfig {
    /// Preset: "fast", "balanced", or "premium"
    pub preset: String,
    /// Per-role model overrides: e.g. {"architect": "qwen2.5-coder:32b", "tester": "qwen3-coder:30b"}
    pub models: BTreeMap<String, String>,
    /// Quality gate threshold override (default: complexity-scaled)
    pub quality_gate: Option<String>,
    /// Enable automatic refinement after quality gate failure
...
impl ConfigLoader {
    ...
    pub fn load(&self) -> Result<RuntimeConfig, ConfigError> {
        ...
        let feature_config = RuntimeFeatureConfig {
            ...
            pipeline: parse_optional_pipeline_config(&merged_value),
        };
...
impl RuntimeConfig {
    ...
    pub fn pipeline(&self) -> Option<&PipelineConfig> {
        self.feature_config.pipeline.as_ref()
    }
}
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".claudette.json");
...
/// Parse the optional `"pipeline"` section from `.claudette.json`.
fn parse_optional_pipeline_config(root: &JsonValue) -> Option<PipelineConfig> {
    let object = root.as_object()?;
    let pipeline_value = object.get("pipeline")?;
    let pipeline = pipeline_value.as_object()?;
    ...
    let quality_gate = pipeline.get("quality_gate").and_then(|v| {
        v.as_str()
            .map(str::to_string)
            .or_else(|| v.as_i64().map(|n| n.to_string()))
    });

    let auto_refine = pipeline
        .get("auto_refine")
