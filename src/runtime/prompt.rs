...
pub const FRONTIER_MODEL_NAME: &str = "Claudette (local)";
...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("CLAUDETTE.md"),
            dir.join("CLAUDETTE.local.md"),
            dir.join(".claudette").join("CLAUDETTE.md"),
            dir.join(".claudette").join("instructions.md"),
        ] {
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No project instruction files loaded.".to_string(),
        ]));
        return lines.join("\n");
    }
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".claudette")).expect("nested claudette dir");
        fs::write(root.join("CLAUDETTE.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("CLAUDETTE.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".claudette")).expect("apps claudette dir");
        fs::write(root.join("apps").join("CLAUDETTE.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".claudette").join("instructions.md"),
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.claudette/CLAUDETTE.md")),
            "CLAUDETTE.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("CLAUDETTE.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? CLAUDETTE.md"));
    ...
    fn load_system_prompt_reads_claude_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".claudette")).expect(" claudette dir");
        fs::write(root.join("CLAUDETTE.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".claudette").join("settings.json"),
...
",
    ...
    fn renders_claude_code_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".claudette")).expect(" claudette dir");
        fs::write(root.join("CLAUDETTE.md"), "Project rules").expect("write CLAUDETTE.md");
        fs::write(
            root.join(".claudette").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
    ...
    fn discovers_dot_claude_instructions_markdown() {
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".claudette/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/CLAUDETTE.md"),
