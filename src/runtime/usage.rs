...
mod tests {
    ...
    fn computes_cost_summary_lines() {
        ...
        let lines = usage.summary_lines_for_model("usage", Some("qwen3.5:4b"));
        assert!(lines[0].contains("estimated_cost=$54.6750"));
        assert!(lines[0].contains("model=qwen3.5:4b"));
        assert!(lines[1].contains("cache_read=$0.3000"));
    }

    #[test]
    fn local_ollama_models_have_no_pricing() {
        // Claudette runs against Ollama — local, free. The pricing lookup is
        // retained for library flexibility but returns None for the models we
        // ship. If someone later adds a paid cloud backend, they'll opt in
        // via a different code path.
        assert!(pricing_for_model("qwen3.5:4b").is_none());
        assert!(pricing_for_model("qwen3-coder:30b").is_none());
        assert!(pricing_for_model("qwen2.5-coder:14b").is_none());
    }
